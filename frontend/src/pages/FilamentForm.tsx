...
export function FilamentForm() {
  ...
  useEffect(() => {
    ...
    const fromGcode = searchParams.get('from_gcode');
    
    if (templateId) {
      loadTemplate(parseInt(templateId));
    } else if (fromGcode === 'true') {
      // Pre-fill from G-code URL parameters
      const materialId = searchParams.get('material_id');
      const colorId = searchParams.get('color_id');
      const name = searchParams.get('name');
      
      if (materialId || colorId || name) {
        setFormData({
          name: name || '',
          material_id: materialId ? parseInt(materialId) : 0,
          color_id: colorId ? parseInt(colorId) : 0,
          manufacturer: '',
          template_id: null,
          notes: '',
        });
  ...
  const loadFilament = async () => {
    try {
      setLoading(true);
      const filament = await filamentsApi.get(parseInt(id!));
      setFormData({
        name: filament.name,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setLoading(true);
      if (isEdit) {
        await filamentsApi.update(parseInt(id!), formData);
      } else {
